/*
 * Copyright 2026 Free Software Foundation, Inc.
 *
 * This file is part of GNU Radio
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 *
 */

/***********************************************************************************/
/* This file is automatically generated using bindtool and can be manually edited  */
/* The following lines can be configured to regenerate this file during cmake      */
/* If manual edits are made, the following tags should be modified accordingly.    */
/* BINDTOOL_GEN_AUTOMATIC(0)                                                       */
/* BINDTOOL_USE_PYGCCXML(0)                                                        */
/* BINDTOOL_HEADER_FILE(mlir_aie_cpp_fft_test.h)                                        */
/* BINDTOOL_HEADER_FILE_HASH(364327a6238d44bfa42dbda36889136f)                     */
/***********************************************************************************/

#include <pybind11/complex.h>
#include <pybind11/pybind11.h>
#include <pybind11/stl.h>

namespace py = pybind11;

#include <gnuradio/mlir_aie/mlir_aie_cpp_fft_test.h>
// pydoc.h is automatically generated in the build directory
#include <mlir_aie_cpp_fft_test_pydoc.h>

void bind_mlir_aie_cpp_fft_test(py::module& m)
{

    using mlir_aie_cpp_fft_test    = gr::mlir_aie::mlir_aie_cpp_fft_test;


    py::class_<mlir_aie_cpp_fft_test, gr::block, gr::basic_block,
        std::shared_ptr<mlir_aie_cpp_fft_test>>(m, "mlir_aie_cpp_fft_test", D(mlir_aie_cpp_fft_test))

        .def(py::init(&mlir_aie_cpp_fft_test::make),
           D(mlir_aie_cpp_fft_test,make)
        )
        



        ;




}








