/* -*- c++ -*- */
/*
 * Copyright 2026 gr-mlir_aie author.
 *
 * SPDX-License-Identifier: GPL-3.0-or-later
 */

#ifndef INCLUDED_MLIR_AIE_MLIR_AIE_CPP_80211_MAGSQ_AND_DIV_H
#define INCLUDED_MLIR_AIE_MLIR_AIE_CPP_80211_MAGSQ_AND_DIV_H

#include <gnuradio/block.h>
#include <gnuradio/mlir_aie/api.h>

namespace gr {
namespace mlir_aie {

/*! 
 * \brief Run the 802.11 AIE magnitude-squared normalization kernel.
 * \ingroup mlir_aie
 *
 * Takes a complex float stream and a float magnitude stream, sends them to the
 * AIE kernel as cbfloat16 and bfloat16 buffers, and emits a float stream.
 */
class MLIR_AIE_API mlir_aie_cpp_80211_magsq_and_div : virtual public gr::block
{
public:
    typedef std::shared_ptr<mlir_aie_cpp_80211_magsq_and_div> sptr;

    /*!
     * \brief Return a shared_ptr to a new instance of
     * mlir_aie::mlir_aie_cpp_80211_magsq_and_div.
     *
     * To avoid accidental use of raw pointers,
     * mlir_aie::mlir_aie_cpp_80211_magsq_and_div's constructor is in a private
     * implementation class. mlir_aie::mlir_aie_cpp_80211_magsq_and_div::make is the
     * public interface for creating new instances.
     */
    static sptr make(const char* path_xclbin,
                     const char* path_insts_bin,
                     const char* kernel_name,
                     int VECTOR_SIZE);
};

} // namespace mlir_aie
} // namespace gr

#endif /* INCLUDED_MLIR_AIE_MLIR_AIE_CPP_80211_MAGSQ_AND_DIV_H */
