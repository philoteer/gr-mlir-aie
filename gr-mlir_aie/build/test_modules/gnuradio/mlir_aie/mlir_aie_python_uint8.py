...
class mlir_aie_python_uint8(gr.basic_block):
    ...
    """
    def __init__(self, path_xclbin="aie-kernel-src/build/final.xclbin",path_insts_bin="aie-kernel-src/build/insts.bin"):
    ...
    def forecast(self, noutput_items, ninputs):
        ninput_items_required = [min(VECTOR_SIZE, noutput_items)] * ninputs
        return ninput_items_required

    def general_work(self, input_items, output_items):
        if(len(input_items[0]) < VECTOR_SIZE):
            return 0
        buffers = [iron.tensor(input_items[0][:VECTOR_SIZE], dtype=DTYPE), self.out_buf] 
        DefaultNPURuntime.run(self.kernel_handle, buffers)        
        output_items[0][:VECTOR_SIZE] = self.out_buf.numpy()
        
        self.consume_each(VECTOR_SIZE)
        return VECTOR_SIZE
